module.exports.config = {
	...
	description: "Check your money or tagged user's money",
...
var argss = `${money}`;
    ...
    ctx.fillText('' + argss.replace(/\B(?=(\d{3})+(?!\d))/g, '.') + ',৳', 530, 359);
    const imageBuffer = canvas.toBuffer();
    fs.writeFileSync(path, imageBuffer);
       var msg =  {body: `[⚜️] Current balance for ${name}\n[⚜️] Your current money: ${money}`, attachment: fs.createReadStream(path)
    }
