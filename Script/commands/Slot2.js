module.exports = {
  ...
  onStart: async function ({ args, message, event, usersData, getLang }) {
    ...
    const winnings = this.win(slot1, slot2, slot3, slot4, slot5, amount);
    ...
    const messageText = this.result(slot1, slot2, slot3, slot4, slot5, winnings, getLang);
    return message.reply(messageText);
  },

  win: function(slot1, slot2, slot3, slot4, slot5, betAmount) {
  ...
  result: function(slot1, slot2, slot3, slot4, slot5, winnings, getLang) {
    const bold = (text) =>
      text
        .replace(/[A-Z]/gi, (c) =>
  ...
  run: async function({ args, message, event, usersData, getLang }) {
    return this.onStart({ args, message, event, usersData, getLang });
  }
};
